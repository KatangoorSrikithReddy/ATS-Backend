...
//         console.log("🔍 Extracted Daaata from Excel:", excelData.slice(0, 5)); // Debug first 5 rows
...
async function importExcelData(filePath, userId) {
    try {
        ...
        // ✅ Sort data by Applicant ID before processing
        excelData.sort((a, b) => {
            const aId = parseInt(a["Applicant ID"], 10);
            const bId = parseInt(b["Applicant ID"], 10);
            return aId - bId;
        });

        console.log("🔍 Extracted and Sorted Data (first 5):", excelData.slice(0, 5));
        console.log(`🚀 Processing ${excelData.length} records...`);
        ...
        for (let row of excelData) {
            ...
            if (typeof applicantId === "string") {
                applicantId = parseInt(applicantId.trim(), 10);
            }
            if (isNaN(applicantId)) {
                console.error(`⚠️ Invalid applicant_id value: ${applicantId}`);
                continue;
